Return-Path: <linux-xfs+bounces-4570-lists+linux-xfs=lfdr.de@vger.kernel.org>
X-Original-To: lists+linux-xfs@lfdr.de
Delivered-To: lists+linux-xfs@lfdr.de
Received: from am.mirrors.kernel.org (am.mirrors.kernel.org [IPv6:2604:1380:4601:e00::3])
	by mail.lfdr.de (Postfix) with ESMTPS id 532AE86F806
	for <lists+linux-xfs@lfdr.de>; Mon,  4 Mar 2024 01:45:44 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by am.mirrors.kernel.org (Postfix) with ESMTPS id C150C1F21248
	for <lists+linux-xfs@lfdr.de>; Mon,  4 Mar 2024 00:45:43 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 1488D33DD;
	Mon,  4 Mar 2024 00:45:39 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (2048-bit key) header.d=kernel.org header.i=@kernel.org header.b="KzshiHkd"
X-Original-To: linux-xfs@vger.kernel.org
Received: from smtp.kernel.org (aws-us-west-2-korg-mail-1.web.codeaurora.org [10.30.226.201])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id B7BB933CE;
	Mon,  4 Mar 2024 00:45:38 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org; arc=none smtp.client-ip=10.30.226.201
ARC-Seal:i=1; a=rsa-sha256; d=subspace.kernel.org; s=arc-20240116;
	t=1709513138; cv=none; b=NJ3oY7xiBkeOBjjgrQNNqO8gOGY0w+XGh/v7E+gpZEJRDy/qSG+Lm+n52rxn/8juWb/kx7GUCrBOd7ZDYgRMBx9S2TPkHBrbhhz7mc8H0T3JlRFf1Di6tWSz0YtyKRnk8q0dsFR8M8LoJOQnNryq+44gSX9tkvvPQv70uX+NJQc=
ARC-Message-Signature:i=1; a=rsa-sha256; d=subspace.kernel.org;
	s=arc-20240116; t=1709513138; c=relaxed/simple;
	bh=uGodwGBnvD5Mu/SLcIhgg4gky1mp+rnbbkD3F6auLvs=;
	h=Message-ID:Subject:From:To:Cc:Date:In-Reply-To:References:
	 Content-Type:MIME-Version; b=CT72ef384qH7Za7JBJUXMkxJ4mmTgudFZwHkQ0kfQiTe6uwFCX94+EbOZx/Nwhl6ng8a/Hd4DI0ESAIyzGd6pFUNtHKuZIyYfmz5pDVEk20rwZabqL3faPA72J4W9OMHhpYTaCw4uU+S07nvlNCB5Y9PxveOD79f1+ZtpOPpw1M=
ARC-Authentication-Results:i=1; smtp.subspace.kernel.org; dkim=pass (2048-bit key) header.d=kernel.org header.i=@kernel.org header.b=KzshiHkd; arc=none smtp.client-ip=10.30.226.201
Received: by smtp.kernel.org (Postfix) with ESMTPSA id D3C96C43394;
	Mon,  4 Mar 2024 00:45:37 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
	s=k20201202; t=1709513138;
	bh=uGodwGBnvD5Mu/SLcIhgg4gky1mp+rnbbkD3F6auLvs=;
	h=Subject:From:To:Cc:Date:In-Reply-To:References:From;
	b=
Message-ID: <b18393ebab290ddd83b22830096796b829568225.camel@kernel.org>
Subject: Re: [PATCH] xfs: stop advertising SB_I_VERSION
From: Jeff Layton <jlayton@kernel.org>
To: Dave Chinner <david@fromorbit.com>
Cc: linux-xfs@vger.kernel.org, djwong@kernel.org,
 linux-fsdevel@vger.kernel.org
Date: Sun, 03 Mar 2024 19:45:36 -0500
In-Reply-To: <ZeUJWuO8TkuoodIx@dread.disaster.area>
References: <20240228042859.841623-1-david@fromorbit.com>
	 <726abff82e992e3e0765e8711e90bf0accb37b2a.camel@kernel.org>
	 <ZeUJWuO8TkuoodIx@dread.disaster.area>
Autocrypt: addr=jlayton@kernel.org; prefer-encrypt=mutual;
 keydata=
Content-Type: text/plain; charset="ISO-8859-15"
Content-Transfer-Encoding: quoted-printable
User-Agent: Evolution 3.50.4 (3.50.4-1.fc39) 
Precedence: bulk
X-Mailing-List: linux-xfs@vger.kernel.org
List-Id: <linux-xfs.vger.kernel.org>
List-Subscribe: <mailto:linux-xfs+subscribe@vger.kernel.org>
List-Unsubscribe: <mailto:linux-xfs+unsubscribe@vger.kernel.org>
MIME-Version: 1.0

On Mon, 2024-03-04 at 10:35 +1100, Dave Chinner wrote:
> On Fri, Mar 01, 2024 at 08:42:17AM -0500, Jeff Layton wrote:
> > On Wed, 2024-02-28 at 15:28 +1100, Dave Chinner wrote:
> > > From: Dave Chinner <dchinner@redhat.com>
> > >=20
> > > The redefinition of how NFS wants inode->i_version to be updated is
> > > incomaptible with the XFS i_version mechanism. The VFS now wants
> > > inode->i_version to only change when ctime changes (i.e. it has
> > > become a ctime change counter, not an inode change counter). XFS has
> > > fine grained timestamps, so it can just use ctime for the NFS change
> > > cookie like it still does for V4 XFS filesystems.
> > >=20
> >=20
> > Are you saying that XFS has timestamp granularity finer than
> > current_time() reports?
>=20
> No.
>=20
> > I thought XFS used the same clocksource as
> > everyone else.
>=20
> It does.
>=20
> > At LPC, you mentioned you had some patches in progress to use the unuse=
d
> > bits in the tv_nsec field as a change counter to track changes that
> > occurred within the same timer tick.
>=20
> Still a possibility, but I wasn't going to do anything in that
> direction because it still seemed like you were still trying to make
> progress down the path of generic timestamp granularity
> improvements.
>=20

Ok, good. I'm glad to hear that that approach isn't unworkable (yet!).

As far as timestamp granularity goes, Linus made it clear that he was
not on board with any approach that would even more complexity than the
original multigrain ctime patches. That pretty much ended my work in
that area.


> > Did that not pan out for some reason? I'd like to understand why if so.
> > It sounded like a reasonable solution to the problem.
>=20
> Time. And the fact that ctime granularity isn't SB_I_VERSION at all,
> so whilst we might support statx change cookies in the future, that
> will not be via a SB_I_VERSION mechanism.
>
> i.e. statx doesn't require us to support SB_I_VERSION for the change
> cookies, so until we are in a position to present a higher
> resolution change cookie via ctime we're just going to remove
> support for both.
>

Yeah, that makes sense.

> > Acked-by: Jeff Layton <jlayton@kernel.org>
>=20
> Thanks!
>=20
> -Dave.

--=20
Jeff Layton <jlayton@kernel.org>

